  ...
  function loadChat(str) {
    ...
    if (str.length == 0) {
      document.getElementById("chat_content").innerHTML = "";
        ...
        xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          current_value=document.getElementById("chat_content").textContent;
          if(current_value!=this.responseText)
  ...
  //sending a message
    ...
    function sendMessage() {
      ...
      if (str.length == 0) {
        ...
        if(currentdate.getHours()<10)  hh="0"+(currentdate.getHours());//add the zero          
        else hh=currentdate.getHours();
        ...
        var time_string = currentdate.getFullYear()+"-"
                  +mm+"-"  
                  +dd +" "
                  + hh + ":"  
                  + minute + ":" 
